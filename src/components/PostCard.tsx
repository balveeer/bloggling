...
const PostCard: React.FC<{ post: PostType, save: boolean }> = ({ post, save=true }) => {
  ...
  return (
    <div className="w-full" ref={cardRef} >
      <Link to={`/post/${post?.$id}`} className="mx-auto relative flex max-w-[20rem] flex-col rounded-xl bg-gray-50 hover:bg-gray-200 dark:bg-[#111111]/70 hover:dark:bg-[#111111] dark:text-white group duration-200 hover:shadow-xl active:scale-95" >
        <img
          //@ts-ignore
        ...
        <div className="w-full">
          <h3 className="mx-4 my-1 min-h-12 pr-16 text-lg md:text-xl font-semibold text-black dark:text-white duration-200 w-full line-clamp-2">
            {post?.title}
          </h3>
          {save && <span
              className="absolute bottom-2 right-2 z-20 rounded-lg"
              onClick={handleSave} 
            >
              {post?.saves.includes(user?.$id) ? (
                <svg
                  ...
                  x="0px"
                  y="0px"
                  className="h-10 w-10 opacity-70 fill-[#111111] dark:fill-white"
                  viewBox="0 0 48 48"
                >
                ...
                <svg
                  ...
                  x="0px"
                  y="0px"
                  className="h-10 opacity-60 fill-[#111111] dark:fill-white"
                  viewBox="0 0 48 48"
                >
