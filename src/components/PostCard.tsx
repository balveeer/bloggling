    ...
    <div className="w-full" ref={cardRef} >
      <Link to={`/post/${post?.$id}`} className="mx-auto relative flex max-w-[20rem] flex-col rounded-xl bg-gray-50 hover:bg-gray-200 bg-clip-border dark:bg-gray-900/70 hover:dark:bg-gray-900 dark:text-white group duration-200 hover:shadow-xl shadow-violet-200 dark:shadow-gray-800 active:scale-95">
        <img
          ...
          loading="lazy"
          alt={post.title}
          className={`relative sm:m-4 sm:h-60 group-hover:rounded-b-none group-hover:m-0 rounded-xl bg-blue-gray-500 bg-clip-border text-white shadow-lg object-cover duration-200 group-hover:mb-4 group-hover:h-[16rem] ${isAtTop ? 'm-0 mb-4 rounded-b-none h-[16rem]' : 'm-4 h-60'}`}
        />
        ...
        <div className="w-full">
          <h3 className="mx-4 my-1 min-h-12 pr-16 text-lg md:text-xl font-semibold text-black dark:text-white duration-200 w-full line-clamp-2 font-serif">
            {post?.title}
          </h3>
          {save && <span
